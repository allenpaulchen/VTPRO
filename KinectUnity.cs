...
{
    public const float prodis = 2f;
    public const float camdis = 1;
    public const float PnCratio = prodis / camdis + prodis % camdis;
    public const float yDist = 0.3f;
}

namespace KinectUnity
{
    public class Kinect
    {
        public const int
        ...
        [DllImport("NitKinectDLL")]
        private static extern IntPtr getKinect();
        [DllImport("NitKinectDLL")]
        private static extern void stopKinect(IntPtr ptr);

        // OpenCV
        [DllImport("NitKinectDLL")]
        private static extern void imshow(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern void imshowBlack(IntPtr ptr);

        // CoordinateMapper
        [DllImport("NitKinectDLL")]
        private static extern void mapCameraPointToColorSpace(IntPtr ptr, IntPtr sv, IntPtr cv, int n);
        [DllImport("NitKinectDLL")]
        private static extern void mapCameraPointToDepthSpace(IntPtr ptr, IntPtr sv, IntPtr dv, int n);
        [DllImport("NitKinectDLL")]
        private static extern void mapDepthPointToColorSpace(IntPtr ptr, IntPtr dv, IntPtr dth, IntPtr cv, int n);
        [DllImport("NitKinectDLL")]
        private static extern void mapDepthPointToCameraSpace(IntPtr ptr, IntPtr dv, IntPtr dth, IntPtr sv, int n);

        // Multi Thread
        [DllImport("NitKinectDLL")]
        private static extern void acquire(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern void release(IntPtr ptr);

        // Audio
        [DllImport("NitKinectDLL")]
        private static extern void setAudio(IntPtr ptr, bool flag);
        [DllImport("NitKinectDLL")]
        private static extern float getBeamAngle(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern float getBeamAngleConfidence(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern ulong getAudioTrackingId(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern void openAudio(IntPtr ptr, IntPtr filename);
        [DllImport("NitKinectDLL")]
        private static extern void closeAudio(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern bool isOpenedAudio(IntPtr ptr);

        // RGB
        [DllImport("NitKinectDLL")]
        private static extern void setRGB(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getRGB(IntPtr ptr, IntPtr data);

        // Depth
        [DllImport("NitKinectDLL")]
        private static extern void setDepth(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getDepth(IntPtr ptr, IntPtr data);

        // Infrared
        [DllImport("NitKinectDLL")]
        private static extern void setInfrared(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getInfrared(IntPtr ptr, IntPtr data);

        // BodyIndex
        [DllImport("NitKinectDLL")]
        private static extern void setBodyIndex(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getBodyIndex(IntPtr ptr, IntPtr data);

        // Skeleton
        [DllImport("NitKinectDLL")]
        private static extern void setSkeleton(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getSkeleton(IntPtr ptr, IntPtr skelton, IntPtr state, IntPtr id, IntPtr tid);
        [DllImport("NitKinectDLL")]
        private static extern int handState(IntPtr ptr, int id, bool isLeft);

        // Face
        [DllImport("NitKinectDLL")]
        private static extern void setFace(IntPtr ptr, bool isColorSpace);
        [DllImport("NitKinectDLL")]
        private static extern int getFace(IntPtr ptr, IntPtr point, IntPtr rect, IntPtr direction, IntPtr property, IntPtr tid);

        // HDFace
        [DllImport("NitKinectDLL")]
        private static extern void setHDFace(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getHDFace(IntPtr ptr, IntPtr point, IntPtr tid, IntPtr status);

        // Gesture
        [DllImport("NitKinectDLL")]
        private static extern void setGestureFile(IntPtr ptr, IntPtr filename);
        [DllImport("NitKinectDLL")]
        private static extern int setGestureId(IntPtr ptr, IntPtr name, int id); // id: non-zero
        [DllImport("NitKinectDLL")]
        private static extern void setGesture(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern int getDiscreteGesture(IntPtr ptr, IntPtr gid, IntPtr confidence, IntPtr tid);
        [DllImport("NitKinectDLL")]
        private static extern int getContinuousGesture(IntPtr ptr, IntPtr gid, IntPtr progress, IntPtr tid);
        [DllImport("NitKinectDLL")]
        private static extern int getGidMapSize();

        // Video
        [DllImport("NitKinectDLL")]
        private static extern void openVideo(IntPtr ptr, IntPtr filename);
        [DllImport("NitKinectDLL")]
        private static extern void writeVideo(IntPtr ptr);
        [DllImport("NitKinectDLL")]
        private static extern void closeVideo(IntPtr ptr);
    ...
    class HumanoidSkeleton
    {
      ...
      Kinect.JointType_KneeLeft, Kinect.JointType_AnkleLeft,             // LeftLowerLeg
      ...
      HumanBodyBones.RightFoot,
        ...
        // move model
        public float PointRotation(Vector3 A, Vector3 B)
        {
        ...
        public void set(Kinect kinect, int n, float correction, bool mirrored = false, bool move = false, bool headMove = true)
        {
            ...
            if (Math.Abs(model_Control.lastRota[n] % 360) < 60 || Math.Abs(model_Control.lastRota[n] % 360) > 300 && model_Control.isback[n])
            {
            ...
            for (int i = 0; i < targetBone.Length; i++)
            {
                ...
                if ((Math.Abs(model_Control.lastRota[n] % 360) < 60 || Math.Abs(model_Control.lastRota[n] % 360) > 300) && i >= 2)
                {
                    trackingSegment[targetBone[i]] = new Vector3(joint[s].x - joint[e].x, joint[e].y - joint[s].y, joint[s].z - joint[e].z);
                }
                else
            ...
            foreach (HumanBodyBones bone in targetBone)
            {
                ...
                {
                    //Quaternion(-0.09666204f, -0.003195104f, -0.03288203f,-0.9947689f);
            ...
            if (mirrored)
            {
                ...
                model_Control.sinceLastAction += Time.deltaTime;
                if (model_Control.sinceLastAction > model_Control.actionTimeout)
                {
                    float[] LeftMidRightX = new float[3];
                    //float[] LeftMidRightY = new float[3];
                    float[] LeftMidRightZ = new float[3];

                    //x
                    LeftMidRightX[0] = kinect.skeleton[n][Kinect.JointType_ShoulderLeft].x;
                    LeftMidRightX[1] = kinect.skeleton[n][Kinect.JointType_SpineShoulder].x;
                    LeftMidRightX[2] = kinect.skeleton[n][Kinect.JointType_ShoulderRight].x;

                    //z
                    LeftMidRightZ[0] = kinect.skeleton[n][Kinect.JointType_ShoulderLeft].z;
                    LeftMidRightZ[1] = kinect.skeleton[n][Kinect.JointType_SpineShoulder].z;
                    LeftMidRightZ[2] = kinect.skeleton[n][Kinect.JointType_ShoulderRight].z;

                    //vector of left -> mid  & right -> mid
                    Vector3 Left2Mid = new Vector3(LeftMidRightX[1] - LeftMidRightX[0], 0, LeftMidRightZ[1] - LeftMidRightZ[0]);
                    Vector3 Right2Mid = new Vector3(LeftMidRightX[1] - LeftMidRightX[2], 0, LeftMidRightZ[1] - LeftMidRightZ[2]);

                    //get angle
                    float dot = Vector3.Dot(Left2Mid.normalized, model_Control.postVector);
                    float angle = Mathf.Acos(dot) * Mathf.Rad2Deg;

                    /*Debug.Log("model_Control.lastRota[n]:  " + model_Control.lastRota[n] +
                        "  Right2Mid.z:  " + Right2Mid.z + "Left2Mid.z:  " + Left2Mid.z);
                    Debug.Log(LeftMidRightZ[2] + ",   " + LeftMidRightZ[0]);*/

                    //bow 
                    if (LeftMidRightZ[0] < kinect.skeleton[n][Kinect.JointType_SpineBase].z && LeftMidRightZ[2] < kinect.skeleton[n][Kinect.JointType_SpineBase].z)
                        model_Control.bow[n] = true;
                    else
                        model_Control.bow[n] = false;


                    //check front or back
                    //edit here
                    if ((model_Control.lastRota[n] < -220 && Right2Mid.z > 0) || (model_Control.lastRota[n] > -140 && Left2Mid.z > 0))
                    {
                        if (model_Control.lastRota[n] != 0 && !model_Control.isback[n])
                        {
                            angle = angle - 180;
                            model_Control.isback[n] = true;
                            Debug.Log("轉到背面了");
                        }
                        else if (model_Control.lastRota[n] != 0 && model_Control.isback[n])
                        {
                            angle = angle + 180;
                            model_Control.isback[n] = false;
                            model_Control.stopcorr[n] = true;
                            Debug.Log("轉到正面了");
                        }
                    }


                    ///correct bool is back
                    ///
                    if (!model_Control.isback[n])
                    {
                        //Debug.Log(Math.Abs(Right2Mid.z - Left2Mid.z));
                        if (model_Control.bow[n])
                            rota = model_Control.lastRota[n];
                        else if (Left2Mid.z > Right2Mid.z)
                        {
                            rota = -angle - 180;
                        }

                        else if (Right2Mid.z > Left2Mid.z)
                            rota = angle - 180;

                        else
                            rota = model_Control.lastRota[n];

                        if (Math.Abs(Right2Mid.z - Left2Mid.z) >= 0.03f)
                            model_Control.correctScale[n] = false;
                        else
                            model_Control.correctScale[n] = true;
                    }
                    else
                    {
                        if (Left2Mid.z > Right2Mid.z)
                        {
                            rota = -angle - 180;
                        }
                        else if (Right2Mid.z > Left2Mid.z)
                    ...
                    //Debug.Log("Left2Mid.normalized:  " + Left2Mid.normalized + "   Mid2Right.normalized:  " + Right2Mid.normalized);
                    //Debug.Log("angleLeft2Mid:  " + angle);
                    //Debug.Log("rota: " + rota);

                    model_Control.lastRota[n] = rota;
                    model_Control.sinceLastAction = 0;
                    //Reverse
                    rota = -180 - (rota - (-180));
                ...
                else
                {
                    ...
                    rota = -180 - (rota - (-180));
                    bodyRot = Quaternion.AngleAxis(rota, Vector3.up) * bodyRot;
                    //  Debug.Log("嗨嗨 :" + model_Control.counterBack + ",    " + model_Control.lastRota);
                }
            }
            if (model_Control.isback[n])
                Debug.Log("旋轉角度 : " + Math.Abs(model_Control.lastRota[n] % 360));
            ...
            if (move == true)
            {
                ...
                //edit model position----------------------
                //1.27 is the max value of x axis, -1.1 is the min value of x axis
                //1.27f - m.x = k2(ppt20181009, P.4)
                //two time correction because of camera and projector both have error angle
                m.x = (m.x - (1.27f - m.x) * model_Control.correctXfirst + (m.x + 1.1f) * model_Control.correctXsecond) * Constants.PnCratio / m.z;
                m.y = m.y * Constants.PnCratio - Constants.yDist;
                //edit model position----------------------
        ...
        new public void set(Kinect kinect, int n, bool mirrored = false, bool move = false, bool headMove = false)
        {
          ...
          * Quaternion.AngleAxis(-90, new Vector3(0, 0, 1));
            ...
            if (move == true)
            {
                ...
                if (mirrored) m = new Vector3(-m.x, m.y, -m.z);
                //edit model position----------------------
                m.x = m.x * Constants.prodis;
                m.y = m.y * Constants.prodis;
                //edit model position----------------------
                humanoid.transform.position = m;
                //                humanoid.transform.Rotate(0, joint[Kinect.JointType_SpineBase].y, 0);
